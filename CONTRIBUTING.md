...
this project displays to be preserved. See [LICENSE](LICENSE) for the licence
...
- Run the suite before opening a pull request: `./mvnw test`. The CI matrix
  builds on Linux and Windows, because nearly every bug this client has had was
  a Windows one.
- Every command's output is somebody reading a terminal. Colour is decoration
  and never information.
- The attribution printed by `orkx --version` and `orkx help` is required by the
  licence. It may not be removed.
